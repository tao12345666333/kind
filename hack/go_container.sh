...
run_in_go_container() {
  ...
  docker run \
    ...
    `# mount the output & source dir, set working directory to the source dir` \
      -v "${OUT_DIR}:/out" -v /go/src/sigs.k8s.io/kind:/go/src/sigs.k8s.io/kind -w "/go/src/sigs.k8s.io/kind" \
  ...
  docker cp /go/src/sigs.k8s.io/kind $(docker ps -ql):/go/src/sigs.k8s.io/
  ...
  mkdir -p bin
  docker cp $(docker ps -ql):/out/kind bin/kind
}
